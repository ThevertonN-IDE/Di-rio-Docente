...
export class ListaView {
  ...
  async render() {
    this.container.innerHTML = `
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl">
              ...
              <div class="flex-1">
                ...
                <input type="file" id="inp-logo-escola" accept="image/*" class="text-xs text-slate-500 file:mr-2 file:py-1 file:px-2.5 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 cursor-pointer">
              </div>
              ${this.vm.dadosCabecalho.logoUrl ? `<button id="btn-remove-logo" class="text-xs text-rose-500 hover:underline">Remover</button>` : ''}
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 text-sm">
              ...
              <input type="text" id="cfg-tipo" value="${this.vm.dadosCabecalho.tipoDocumento}" placeholder="Título (ex: LISTA 01 - TRIGONOMETRIA)" class="border p-2 rounded-lg text-xs">
            </div>
          </div>

          <!-- Controles de Layout e Espaçamento Global -->
          <div class="grid grid-cols-2 gap-3">
            <div class="flex flex-col gap-1 bg-slate-50 p-3 rounded-xl border border-slate-200">
              <span class="text-xs font-bold text-slate-600">Layout de Colunas:</span>
              <div class="flex gap-2 mt-1">
                <button id="btn-col-1" class="flex-1 py-1 rounded-lg text-xs font-bold ${!this.vm.duasColunas ? 'bg-indigo-600 text-white' : 'bg-white border text-slate-600'}">1 Coluna</button>
                <button id="btn-col-2" class="flex-1 py-1 rounded-lg text-xs font-bold ${this.vm.duasColunas ? 'bg-indigo-600 text-white' : 'bg-white border text-slate-600'}">2 Colunas</button>
              </div>
            </div>

            <div class="flex flex-col gap-1 bg-slate-50 p-3 rounded-xl border border-slate-200">
              <span class="text-xs font-bold text-slate-600">Espaço Padrão (Linhas):</span>
              <div class="flex items-center gap-2 mt-1">
                <input type="number" id="inp-espaco-global" min="0" max="25" value="${this.vm.espacoGlobal}" class="w-16 border rounded-lg p-1 text-center text-xs bg-white font-bold">
                <button id="btn-aplicar-espaco" class="px-2 py-1 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-lg text-xs font-semibold">Aplicar a Todas</button>
              </div>
            </div>
          </div>

          <!-- Ações Rápidas de Questões -->
          <div class="space-y-4">
            <div class="flex items-center justify-between">
              <h3 class="text-xs font-bold text-slate-500 uppercase">Questões (${this.vm.questoes.length})</h3>
              <div class="flex items-center gap-2">
                <button id="btn-abrir-banco" class="text-xs text-indigo-600 hover:underline font-bold">📚 Banco Central</button>
      ...
      <div id="modal-banco-lista" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center hidden p-4">
        <div class="bg-white border border-slate-200 rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-4 max-h-[85vh] flex flex-col">
          <div class="flex items-center justify-between border-b pb-3">
            ...
            <button id="btn-fechar-banco-lista" class="text-slate-400 hover:text-slate-600 text-lg">&times;</button>
          </div>
          <input type="text" id="inp-filtro-banco-lista" placeholder="Filtrar por assunto..." class="w-full border rounded-lg p-2 text-xs">
        ...
        <div class="flex items-center justify-between text-xs font-bold text-slate-600">
          ...
          <div class="flex items-center gap-3">
            <label class="flex items-center gap-1 font-normal text-slate-600">
              Espaçamento:
              <input type="number" min="0" max="30" data-q-espaco="${idx}" value="${q.linhasEspaco}" class="w-14 text-center border rounded p-1 text-xs bg-white font-bold">
              linhas
            </label>
            <button data-remove-q="${idx}" class="text-rose-500 hover:text-rose-700 font-bold">Excluir</button>
          </div>
        </div>
